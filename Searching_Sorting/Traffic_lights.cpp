...
void solve() {
    ...
    map<ll, ll> mp;  // Stores segment boundaries
    mp[0] = x;
    
    ll maxi = x;  // Track maximum segment length
    map<ll, ll> a;  // Stores frequency of segment lengths
    a[x] = 1;
    ...
    for (int i = 0; i < n; i++) {
        ...
        ll first = prev(it)->first, second = prev(it)->second;
        
        // Remove old segment
        mp.erase(first);
        a[second - first]--;
        if (a[second - first] == 0) {
            a.erase(second - first);
        }
